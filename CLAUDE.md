...
### BOS Alignment in Probing and Patching (`interpret/interpreters/base_interpreter.py`)
`tracer.invoke(prompt)` runs the tokenizer in full call-mode, which prepends BOS for Llama-family tokenizers (Llama 3.2, Llama-2 / OpenELM). But labels are built against `tokenizer.tokenize(prompt)`, which does not add special tokens. Without compensation, hidden states at position `i` correspond to *one earlier* action than the label at row `i`, every probe trains on noise, and the resulting per-layer accuracy curve is flat at chance (~1/6 for state on S3, ~1/2 for parity). GPT-2 / Pythia / RWKV don't prepend BOS, so the paper's original results are unaffected. `eval.py:180-181` already strips this BOS column from predictions; the interpreter code now mirrors that convention. `BaseInterpreter.setup_model` computes `self.bos_offset` once by tokenizing a probe string with `add_special_tokens=True` and checking whether the first id equals `bos_token_id`; `probing_interpreter.get_layer_representations` and `activation_patching_interpreter.get_patching_results_pair` add this offset to every index taken against `tokenizer.tokenize()` before slicing into traced activations. If a future family's tokenizer prepends more than one special token (or a non-BOS token), extend the detection here.
